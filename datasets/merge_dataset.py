...
class MergedProteinLigandData(Dataset):
    def __init__(self, dataset1, dataset2):
        ...
        for path in possible_paths:
            affinity_path = path + 'affinity_info.pkl'
            chem_path = path + 'chem_info.pkl'

            if os.path.exists(affinity_path) and os.path.exists(chem_path):
                affinity_file = affinity_path
                chem_file = chem_path
                break

        if affinity_file is None or chem_file is None:
            raise FileNotFoundError(
                f"Could not find affinity_info.pkl and chem_info.pkl in any of these paths: {possible_paths}"
            )
