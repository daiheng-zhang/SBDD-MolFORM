...
class ScorePosNet3D(nn.Module):
    ...
    def __init__(self, config, protein_atom_feature_dim, ligand_atom_feature_dim):
        ...
        # Confidence heads removed. Keep a soft-compat check for old configs.
        self.add_output = list(getattr(config, "add_output", []) or [])
        if "confidence" in self.add_output:
            warnings.warn(
                "Config requests confidence head (add_output includes 'confidence'), "
                "but confidence heads are disabled in this version. Ignoring."
            )
    ...
    def chamfer_distance_per_mol(self, x, y, batch_ligand):
        ...
        for i in range(num_mols):
            ...
            y_i = y[batch_ligand == i]
            if x_i.size(0) == 0 or y_i.size(0) == 0:
                chamfer_list.append(torch.tensor(0.0, device=x.device))
                continue
            x_i = x_i.unsqueeze(1)  # [n1, 1, 3]
            y_i = y_i.unsqueeze(0)  # [1, n2, 3]
            dist = torch.norm(x_i - y_i, dim=2)  # [n1, n2]
            cd_xy = dist.min(dim=1)[0].mean()
            cd_yx = dist.min(dim=0)[0].mean()
            chamfer_list.append(cd_xy + cd_yx)
        return torch.stack(chamfer_list)

    def self_normalized_x0_regression(
        self,
        x_pred,
        x0,
        batch_ligand,
        ...
        """
        DiffusionNFT self-normalized x0 regression:
            ||x_pred - x0||^2 / sg(mean(|x_pred - x0|))
        sg(.) is detach().

        If per_mol=True: returns [num_mols] loss vector; else returns scalar.
        """
        if batch_ligand.numel() == 0:
            if per_mol:
                return torch.zeros(0, device=x_pred.device)
            return torch.tensor(0.0, device=x_pred.device)

        diff = x_pred - x0  # [N, 3]
        per_atom_l2 = diff.pow(2).sum(dim=-1)  # [N]
        per_atom_l1 = diff.abs().mean(dim=-1)  # [N]

        per_mol_l2 = scatter_mean(per_atom_l2, batch_ligand, dim=0)
        per_mol_l1 = scatter_mean(per_atom_l1, batch_ligand, dim=0).detach()
        per_mol_l1 = torch.clamp(per_mol_l1, min=eps)

        loss_per_mol = per_mol_l2 / per_mol_l1
        return loss_per_mol if per_mol else loss_per_mol.mean()
    ...
    def get_diffusion_loss_nft(
        ...
        """
        DiffusionNFT-style loss with implicit pos/neg branches.

        reward: tensor of shape (num_mols,) in [0, 1]
        old_model: frozen/EMA model for v_old
        """
        protein_pos, ligand_pos, _ = center_pos(
            protein_pos,
            ligand_pos,
        ...
        ligand_pos_xt_raw, ligand_atom_feature_full_xt_raw, t = self.modify_input_With_time(
            ligand_pos, ligand_v, batch_ligand
        )
        ligand_pos_xt = self.mlp_pos(ligand_pos_xt_raw, t)
        ligand_atom_feature_full_xt = self.mlp_elem(ligand_atom_feature_full_xt_raw, t)
        ...
        with torch.no_grad():
            old_model.eval()
            ligand_pos_xt_old = old_model.mlp_pos(ligand_pos_xt_raw, t)
            ligand_atom_feature_full_xt_old = old_model.mlp_elem(
                ligand_atom_feature_full_xt_raw, t
            )
            preds_old = old_model(
                protein_pos=protein_pos,
                protein_v=protein_v,
                batch_protein=batch_protein,
                init_ligand_pos=ligand_pos_xt_old,
                init_ligand_v=ligand_atom_feature_full_xt_old,
                batch_ligand=batch_ligand,
            )
            pred_ligand_pos_old = preds_old["pred_ligand_pos"]
            pred_ligand_v_old = preds_old["pred_ligand_v"]

        v_pos = (1.0 - beta_nft) * pred_ligand_pos_old + beta_nft * pred_ligand_pos
        v_neg = (1.0 + beta_nft) * pred_ligand_pos_old - beta_nft * pred_ligand_pos

        per_mol_pos = self.self_normalized_x0_regression(
            x_pred=v_pos,
            x0=ligand_pos,
            batch_ligand=batch_ligand,
            eps=1e-3,
        ...
        per_mol_neg = self.self_normalized_x0_regression(
            x_pred=v_neg,
            x0=ligand_pos,
            batch_ligand=batch_ligand,
            eps=1e-3,
        ...
        reward = reward.view(-1).to(per_mol_pos.device)
        if reward.numel() != per_mol_pos.numel():
            raise ValueError(
                f"reward shape mismatch: reward.numel()={reward.numel()} vs "
                f"per_mol_pos.numel()={per_mol_pos.numel()}"
            )
        loss_pos = (reward * per_mol_pos + (1.0 - reward) * per_mol_neg).mean()

        chamfer_pos = self.chamfer_distance_per_mol(v_pos, ligand_pos, batch_ligand)
        chamfer_neg = self.chamfer_distance_per_mol(v_neg, ligand_pos, batch_ligand)
        if chamfer_pos.numel() > 0:
            chamfer_loss = (reward * chamfer_pos + (1.0 - reward) * chamfer_neg).mean()
        else:
            chamfer_loss = torch.tensor(0.0, device=ligand_pos.device)

        beta_v = beta_nft if beta_nft_v is None else beta_nft_v
        v_pos_logits = (1.0 - beta_v) * pred_ligand_v_old + beta_v * pred_ligand_v
        v_neg_logits = (1.0 + beta_v) * pred_ligand_v_old - beta_v * pred_ligand_v
        per_atom_v_pos = F.cross_entropy(v_pos_logits, ligand_v, reduction="none")
        per_atom_v_neg = F.cross_entropy(v_neg_logits, ligand_v, reduction="none")
        per_mol_v_pos = scatter_mean(per_atom_v_pos, batch_ligand, dim=0)
        per_mol_v_neg = scatter_mean(per_atom_v_neg, batch_ligand, dim=0)
        loss_v = (reward * per_mol_v_pos + (1.0 - reward) * per_mol_v_neg).mean()
        loss = loss_pos + loss_v * self.loss_v_weight + self.chamfer_loss_weight * chamfer_loss
    ...
    def sample_diffusion(
        ...
        ligand_pos = ligand_pos + offset[batch_ligand]
        out = {"pred_pos": ligand_pos, "pred_v": ligand_v}
        return out
