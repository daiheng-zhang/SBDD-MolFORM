...
def eval_sample(example_idx, sample_idx, pred_pos, pred_v, ligand_filename,
    ...
    try:
        ...
        return pred_atom_type, r_stable, pair_dist, recon_success, complete, eval_success, bond_dist, bond_angle, dic
    recon_success = 1
...
def save_result_to_file(results, save_path:str=None):
    ...
    for idx, result in enumerate(results):
        if idx not in result_dict:
            result_dict[idx] = {}
        pred_atom_type, r_stable, pair_dist, recon_success, complete, eval_success, bond_dist, bond_angle, dic = result
