...
def sample_diffusion_ligand(model:ScorePosNet3D,
    ...
    if return_confidence:
        warnings.warn(
            "return_confidence=True requested, but confidence heads are disabled. "
    ...
    if return_confidence:
        none_list = [None] * len(all_pred_pos)
        return (
            all_pred_pos,
            all_pred_v,
            all_pred_pos_traj,
...
if __name__ == '__main__':
    ...
    if missing_keys or unexpected_keys:
        raise RuntimeError(
            f"Error(s) in loading state_dict for ScorePosNet3D:\n"
    ...
    pred_pos, pred_v, pred_pos_traj, pred_v_traj = sample_out
