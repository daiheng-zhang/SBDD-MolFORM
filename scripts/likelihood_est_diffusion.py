...
if __name__ == '__main__':
    ...
    # Load checkpoint
    ckpt = misc.load_checkpoint(config.model.checkpoint, map_location=args.device)
    ...
    ligand_featurizer = trans.FeaturizeLigandAtom(dataset_config.transform.ligand_atom_mode)
    transform_list = [
        protein_featurizer,
        ligand_featurizer,
        trans.FeaturizeLigandBond(),
    ]
    if dataset_config.transform.random_rot:
    ...
    state_dict = _strip_confidence_state_dict(ckpt['model'])
    model.load_state_dict(state_dict, strict=False if 'train_config' in config.model else True)
